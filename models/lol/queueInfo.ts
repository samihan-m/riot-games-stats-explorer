...
export const defaultQueueInfo: QueueInfo[] = [
    {
        "queueId": 0,
        "map": "Custom games",
        "description": null,
...
export async function getAllQueueInfo(): Promise<QueueInfo[]> {
    const queueInfoUrl = "https://static.developer.riotgames.com/docs/lol/queues.json";
    let queueInfoResponse = await fetch(queueInfoUrl);
    if(queueInfoResponse.ok === false) {
        return defaultQueueInfo;
    }
    let queueInfo: QueueInfo[] = await queueInfoResponse.json();
    return queueInfo;
}

export function getQueueInfo(queueId: number, allQueueInfo: QueueInfo[]): QueueInfo {
